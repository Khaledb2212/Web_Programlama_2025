...
//        // -------------------------
//        // OPTIONAL: List available models (useful when model names differ by account/quota)
//        // -------------------------
//        public async Task<string> ListModelsRaw()
//        {
...
//            var prompt =
//                "Analyze this person's body type visually. Provide a strict but sustainable diet and exercise plan. " +
//                "Also write a short vivid paragraph describing how this person will look after 6 months of following the plan.";

//            // Keep your working snake_case format here (it already gave you text results)
//            var requestBody = new
//            {
//                contents = new[]
//                {
...
//            var prompt =
//                $"Act as an expert fitness coach. My stats are Height: {height}cm, Weight: {weight}kg, Age: {age}. " +
//                "Provide a detailed diet and exercise plan. Also write a short vivid paragraph describing how I will look after 6 months.";
...
//        // IMAGE EDITING: Future self from uploaded photo (Nano Banana)
//        // (Edits made here:
//        //  1) Use inlineData + mimeType (camelCase) for image models
//        //  2) Force responseModalities: ["Image"]
//        //  3) Make the prompt more explicit/measurable so changes are visible
//        //  4) Add role="user"
//        // )
//        // -------------------------
//        public async Task<byte[]?> GenerateFutureSelfPreviewFromPhoto(byte[] imageBytes, string mimeType)
...
//            var prompt =
//                "Generate an EDITED version of the provided photo. Keep the SAME person (same face identity) and do NOT change the face. " +
//                "Make these visible changes: flatter abdomen, smaller waist, improved posture, slightly broader shoulders, more athletic body shape. " +
//                "Keep clothing, lighting, and background as similar as possible. Photorealistic. " +
//                "Return ONLY the edited image (no text).";
...
//                            // IMPORTANT: For image generation/editing docs, use inlineData + mimeType (camelCase).
//                            // This avoids cases where the request is accepted but the model returns a "no-op" image.
...
//        // -------------------------
//        // SANITY TEST: Very obvious edit (use once to confirm the pipeline is truly editing)
//        // If this returns the same image, you're either displaying the wrong image,
//        // caching the same URL, or the API is returning a no-op.
//        // -------------------------
//        public async Task<byte[]?> GenerateSanityEdit_AddRedCap(byte[] imageBytes, string mimeType)
//        {
//            var base64Image = Convert.ToBase64String(imageBytes);

//            var prompt =
//                "Edit the provided photo by adding a bright red baseball cap on the person's head. " +
//                "Keep the same person, same background, photorealistic. Return ONLY the image (no text).";
...
//            var prompt =
//                $"Create a photorealistic full-body portrait of a person who is {height}cm, {weight}kg, age {age}, " +
//                "then show how they might look after 6 months of consistent diet and gym. " +
//                "Keep it respectful, fully clothed, and realistic. Return ONLY the image (no text).";
...
//                {
//                    // Most image responses return inlineData (camelCase)
//                    if (part.TryGetProperty("inlineData", out var inlineData) &&
//                        inlineData.TryGetProperty("data", out var dataEl1))
//                    {
//                        var b64 = dataEl1.GetString();
...
//                    // Keep snake_case fallback just in case
//                    if (part.TryGetProperty("inline_data", out var inline_data) &&
//                        inline_data.TryGetProperty("data", out var dataEl2))
//                    {
//                        var b64 = dataEl2.GetString();
